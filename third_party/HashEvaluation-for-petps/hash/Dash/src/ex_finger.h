...
class Finger_EH : public Hash<T> {
 public:
  ...
  ~Finger_EH(void);
  inline bool Insert(T key, Value_t value);
  ...
  hash_Utilization utiliz() {
    ...
    hash_Utilization h;
    h.load_factor = 100.0 * (float)_count /
                    float(seg_count * kNumPairPerBucket * (kNumBucket + 2));
    h.utilization =
        100.0 * (float)_count * 16 /
        float(seg_count * sizeof(Table<T>) + sizeof(Table<T> **) * capacity);
    return h;
#endif
    return hash_Utilization();
  }
  void getNumber() {
...
void Finger_EH<T>::Directory_Doubling(int x, Table<T> *new_b, Table<T> *old_b) {
  ...
  // std::cout << "Directory_Doubling towards " << global_depth + 1 <<
  // std::endl;
...
template <class T>
bool Finger_EH<T>::Insert(T key, Value_t value, bool is_in_epoch) {
  if (!is_in_epoch) {
...
template <class T>
bool Finger_EH<T>::Insert(T key, Value_t value) {
  bool resize = false;
  uint64_t key_hash;
  if constexpr (std::is_pointer_v<T>) {
