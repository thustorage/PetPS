...
class Linear : public Hash<T> {
 public:
  ...
  bool Delete(T);
  bool Insert(T key, Value_t value, bool);
  bool Delete(T, bool);
  ...
  bool recoverSegment(Table<T> **seg_ptr, size_t, size_t, size_t);
  std::string hash_name() { return "Dash_lh"; }
  float utiliz() {
    uint64_t count = 0;
    ...
    for (int i = 0; i < occupied_bucket; ++i) {
      ...
      overflowBucket<T> *next_bucket = curr_table->stash->next;
      while (next_bucket != NULL) {
    ...
    // std::cerr << count << "  " << Bucket_num * kNumPairPerBucket <<
    // std::endl;
    return (float)count / (Bucket_num * kNumPairPerBucket);
  }
  ...
  void getNumber() {
    ...
    std::cout << "the local load factor = "
              << (double)count / (Bucket_num * kNumPairPerBucket) << std::endl;
    std::cout << "the local raw sapce utilization = "
              << (double)count / (Bucket_num * 16) << std::endl;
...
template <class T>
bool Linear<T>::recoverSegment(Table<T> **seg_ptr, size_t index, size_t dir_idx,
                               size_t offset) {
RETRY:
...
template <class T>
bool Linear<T>::Insert(T key, Value_t value, bool is_in_epoch) {
  if (!is_in_epoch) {
...
template <class T>
bool Linear<T>::Insert(T key, Value_t value) {
  bool resize = false;
  uint64_t key_hash;
  if constexpr (std::is_pointer_v<T>) {
