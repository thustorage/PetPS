		...
		namespace experimental
		{
			...
			public:
				...
				struct ret
				{
					...
					ret(size_type _level_idx, difference_type _bucket_idx, 
						size_type _slot_idx, uint64_t _nvmps_value_ptr,
						 bool _expanded = false, uint64_t _cap = 0)
						: found(true), level_idx(_level_idx), bucket_idx(_bucket_idx),
						  slot_idx(_slot_idx), expanded(_expanded), capacity(_cap)
					{
						nvmps_value_ptr = _nvmps_value_ptr;
					}
					...
					do
					{
						...
						for (size_type j = 0; j < assoc_num; j++)
						{
							if (f_b.slots[j].x.partial == partial && f_b.slots[j].p.get_offset() != 0)
							{
								...
								{
									// std::cout << f_b.slots[j].p.get_address(my_pool_uuid)->first << " <<<<<<<<<<<<<" << f_b.slots[j].p.get_address(my_pool_uuid)->second << std::endl;
									return ret(i, f_idx, j, f_b.slots[j].p.get_address(my_pool_uuid)->second);
								}
							}
						}
						...
						for (size_type j = 0; j < assoc_num; j++)
						{
							if (s_b.slots[j].x.partial == partial && s_b.slots[j].p.get_offset() != 0)
							{
								...
								{
									// std::cout << f_b.slots[j].p.get_address(my_pool_uuid)->first << " <<<<<<<<<<<<<" << f_b.slots[j].p.get_address(my_pool_uuid)->second;
									return ret(i, s_idx, j, s_b.slots[j].p.get_address(my_pool_uuid)->second);
								}
							}
						}
