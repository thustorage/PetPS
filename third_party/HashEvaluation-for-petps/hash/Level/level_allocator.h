...
namespace level_allocator {

static const char* layout_name = "clht_hashtable";
static const constexpr uint64_t pool_addr = 0x6f0000000000;
...
class Allocator {
 public:
  static void Initialize(const char* pool_name, size_t pool_size) {
    ...
    if (!level_allocator::FileExists(pool_name)) {
    ...
    std::cout << "pool opened at: " << std::hex << allocator_->pm_pool_
              << std::dec << std::endl;
    level_allocator::flush(allocator_->pm_pool_);
  }
