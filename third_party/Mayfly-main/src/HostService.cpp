...
HostService::HostService(DSM *dsm, uint32_t server_thread_nr)
    : dsm(dsm), server_thread_nr(server_thread_nr), is_run(false) {

  index = new Index(dsm->get_dsm_base_addr());
  printf("Host allocates %ldMB for index\n",
         index->index_bucket_size() / define::MB);
}

void HostService::run() {

  Debug::notifyInfo("Host Service start...");

  for (size_t i = 0; i < server_thread_nr; ++i) {
    kv_thread[i] = std::thread(&HostService::run_thread, this, i);
  }
...
uint64_t host_counter[64][8];
void HostService::stat() {
...
void HostService::run_thread(int id) {
  ...
  while (true) {
    ...
    host_counter[dsm->getMyThreadID()][0]++;
