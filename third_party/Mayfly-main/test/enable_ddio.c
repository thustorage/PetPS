...
// gcc enable_ddio.c -o enable_ddio -lpci
...
struct pci_dev *find_ddio_device(uint8_t nic_bus) {
  ...
  for (dev = pacc->devices; dev; dev = dev->next) {
     ...
     * different
     */
    if (/* dev->func == 2 && dev->dev == 0  && */ pci_read_byte(
...
void enable_ddio_for_nic(uint8_t nic_bus) {
...
int main(void) {
  ...
  uint8_t nic_bus_0 = 0x31;
  uint8_t nic_bus_1 = 0xb1;

  enable_ddio_for_nic(nic_bus_0);
  enable_ddio_for_nic(nic_bus_1);
